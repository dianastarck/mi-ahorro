...
class _HomePageState extends State<HomePage> {
  ...
  double get usd => double.tryParse(incomeUsd.text.replaceAll(',', '.')) ?? 0;
  double get rate => double.tryParse(exchangeRate.text.replaceAll(',', '.')) ?? 0;
  double get incomeArs => usd * rate;
  double get fixedTotal => fixed.fold(0.0, (sum, item) => sum + ((item['amount'] as num?)?.toDouble() ?? 0));
  double get paidTotal => fixed.where((e) => e['paid'] == true).fold(0.0, (sum, item) => sum + ((item['amount'] as num?)?.toDouble() ?? 0));
  ...
  Future<void> loadMonth() async {
    ...
    try {
      ...
      // La app igual abre aunque la tabla todavía no tenga todas las columnas.
    }
    if (mounted) setState(() => loading = false);
  }
  ...
  Future<void> saveMonth() async {
    try {
      ...
      await supabase.from('finance_months').upsert({
        ...
        'exchange_rate': rate,
        'income_usd': usd,
        'real_saving_usd': double.tryParse(realSavingUsd.text.replaceAll(',', '.')) ?? 0,
        'notes': notes.text,
  ...
  Widget inicio() => base(Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        monthSelector(),
        const SizedBox(height: 16),
        ...
        const SizedBox(height: 18),
        LinearProgressIndicator(value: fixedTotal == 0 ? 0 : (paidTotal / fixedTotal).clamp(0, 1)),
        const SizedBox(height: 8),
        Text('Pagaste ${fixedTotal == 0 ? 0 : ((paidTotal / fixedTotal) * 100).toStringAsFixed(0)}% de tus gastos fijos'),
      ]));
  ...
  Widget gastos() => base(Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          return Card(
            child: ListTile(
              ...
              subtitle: TextField(
                ...
                onChanged: (v) => fixed[i]['amount'] = double.tryParse(v.replaceAll(',', '.')) ?? 0,
              ),
        ...
        const SizedBox(height: 12),
        FilledButton.icon(
          onPressed: () => setState(() => fixed.add({'icon': '🧾', 'name': 'Nuevo gasto', 'amount': 0.0, 'paid': false})),
  ...
  Widget semana() {
    ...
    final daily = days == 0 ? 0 : available / days;
    ...
    return base(Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      ...List.generate(5, (i) {
        ...
        return Card(child: ListTile(title: Text('Semana ${i + 1}: día $start al $end'), subtitle: Text('Tope estimado: ${money(daily * (end - start + 1))}')));
      }),
    ]));
  }
  ...
  Widget monthSelector() => Row(children: [
        Expanded(child: DropdownButtonFormField<int>(value: month, decoration: const InputDecoration(labelText: 'Mes'), items: List.generate(12, (i) => DropdownMenuItem(value: i + 1, child: Text(DateFormat.MMMM('es').format(DateTime(2026, i + 1))))), onChanged: (v) { if (v != null) { setState(() => month = v); loadMonth(); } })),
